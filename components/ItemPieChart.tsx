...
interface Props {
  ...
  total: number;
  title?: string;
  /** Unit shown in tooltip/legend — "ชิ้น" by default, but e.g. "เซต" for
   * reports like "สถิติล้าง" whose quantity actually tracks sets. */
  unit?: string;
}

export default function ItemPieChart({
  data,
  total,
  title = "สัดส่วนจำนวนรายการ",
  unit = "ชิ้น",
}: Props) {
  const sorted = [...data].sort((a, b) => b.quantity - a.quantity);
  const top7 = sorted.slice(0, 7);
  const otherCount = sorted.slice(7).reduce((s, d) => s + d.quantity, 0);

  const chartData = [
    ...top7.map((d) => ({ name: d.name, count: d.quantity })),
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      const pct = total ? ((payload[0].value / total) * 100).toFixed(2) : "0";
      return (
        <div className="bg-white border border-gray-200 rounded-lg shadow-lg px-3 py-2 text-sm">
          <p className="font-medium text-gray-700">{payload[0].name}</p>
          <p className="text-blue-600 font-bold">
            {formatNumber(payload[0].value)} {unit} ({pct}%)
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-md overflow-hidden">
      <ReportSectionHeader title={title} icon={PieChartIcon} />
      <div className="p-5">
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 space-y-1 min-w-0">
          {chartData.map((item, index) => {
            const pct = total ? ((item.count / total) * 100).toFixed(2) : "0";
            return (
          ...
          {otherCount > 0 && (
            <ReportNote className="text-[10px] mt-1 leading-tight">
              * อื่น ๆ คือรายการที่เหลือรวมกัน
            </ReportNote>
          )}
        </div>
      </div>
